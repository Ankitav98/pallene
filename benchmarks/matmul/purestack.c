...
static Table * function_matmul_titan(
    ...
    L->top += 3;
    #define A_SLOT s2v(frame+0)
    #define B_SLOT s2v(frame+1)
    #define C_SLOT s2v(frame+2)

    #define A_VALUE hvalue(A_SLOT)
    #define B_VALUE hvalue(A_SLOT)
    #define C_VALUE hvalue(A_SLOT)

    sethvalue(L, A_SLOT, x1);
    sethvalue(L, B_SLOT, x2);

    {
        ...
        sethvalue(L, C_SLOT, x7);

        lua_Integer x10 = luaH_getn(A_VALUE);
        lua_Integer x11 /* NI */ = x10;
        lua_Integer x12 = luaH_getn(B_VALUE);
        lua_Integer x13 /* NK */ = x12;
        lua_Unsigned x14 /* ui */ = ((lua_Unsigned)1) - 1;
        if (TITAN_UNLIKELY(x14 >= B_VALUE->sizearray)) {
            titan_renormalize_array(L, B_VALUE, x14, 5);
        }
        const TValue * x15 /* arrslot */ = &B_VALUE->array[x14];
        ...
        while ((x21 >= 0 ? x19 <= x20 : x19 >= x20)) {
            ...
            (void) x22;
            {
                ...
                if (TITAN_UNLIKELY(x33 >= C_VALUE->sizearray)) {
                    titan_renormalize_array(L, C_VALUE, x33, 11);
                }
                TValue * x34 /* slot */ = &C_VALUE->array[x33];
                sethvalue(L, x34, x26);
                if (isblack(obj2gco(C_VALUE)) && iswhite(obj2gco(x26))) {
                    luaC_barrierback_(L, obj2gco(C_VALUE));
                }
            }
            x19 = intop(+, x19, x21);
        }
        ...
        while ((x37 >= 0 ? x35 <= x36 : x35 >= x36)) {
            ...
            (void) x38;
            {
                ...
                if (TITAN_UNLIKELY(x39 >= B_VALUE->sizearray)) {
                    titan_renormalize_array(L, B_VALUE, x39, 14);
                }
                const TValue * x40 /* arrslot */ = &B_VALUE->array[x39];
                ...
                while ((x45 >= 0 ? x43 <= x44 : x43 >= x44)) {
                    ...
                    (void) x46;
                    {
                        ...
                        if (TITAN_UNLIKELY(x47 >= A_VALUE->sizearray)) {
                            titan_renormalize_array(L, A_VALUE, x47, 16);
                        }
                        const TValue * x48 /* arrslot */ = &A_VALUE->array[x47];
                        ...
                        if (TITAN_UNLIKELY(x54 >= C_VALUE->sizearray)) {
                            titan_renormalize_array(L, C_VALUE, x54, 17);
                        }
                        const TValue * x55 /* arrslot */ = &C_VALUE->array[x54];
                        ...
                        while ((x60 >= 0 ? x58 <= x59 : x58 >= x59)) {
                            ...
                            (void) x61;
                            {
                                ...
                                if (TITAN_UNLIKELY(x65 >= x42->sizearray)) {
                                    titan_renormalize_array(L, x42, x65, 19);
                                }
                                ...
                                if (TITAN_UNLIKELY(x70 >= x57->sizearray)) {
                                    titan_renormalize_array(L, x57, x70, 19);
                                }
...
int luaopen_benchmarks_matmul_purestack(lua_State *L)
{
    lua_checkstack(L, 4);

...

    {
        ...
        while ((x10 >= 0 ? x7 <= x9 : x7 >= x9)) {
            ...
            (void) x11;
            {
                ...
                if (TITAN_UNLIKELY(x12 >= x1->sizearray)) {
                    titan_renormalize_array(L, x1, x12, 4);
                }
    ...
    (void)x5;
    {
        ...
        while ((x8 >= 0 ? x6 <= x7 : x6 >= x7)) {
            ...
            (void) x9;
            {
                ...
                if (TITAN_UNLIKELY(x10 >= x2->sizearray)) {
                    titan_renormalize_array(L, x2, x10, 18);
                }
...
#define A_VALUE hvalue(A_SLOT)
    ...
    sethvalue(L, A_SLOT, x2);

    {
        {
            int x17 = x3 > x1;
            if (x17) {
                function_printsolution_titan(L, x1, A_VALUE);
            } else {
                lua_Integer x7 /* start */ = 1;
                ...
                while ((x9 >= 0 ? x7 <= x8 : x7 >= x8)) {
                    ...
                    {
                        {
                            int x12 = function_isplaceok_titan(L, A_VALUE, x3, x10);
                            if (x12) {
                                lua_Unsigned x13 /* ui */ = ((lua_Unsigned)x3) - 1;
                                if (TITAN_UNLIKELY(x13 >= A_VALUE->sizearray)) {
                                    titan_renormalize_array(L, A_VALUE, x13, 40);
                                }
                                TValue * x14 /* slot */ = &A_VALUE->array[x13];
                                setivalue(x14, x10);
                                lua_Integer x15 = intop(+, x3, 1);
                                function_addqueen_titan(L, x1, A_VALUE, x15);
                            }
                        }
                    }
                    x7 = intop(+, x7, x9);
...
int luaopen_benchmarks_queen_purestack(lua_State *L)
{
    lua_checkstack(L, 4);

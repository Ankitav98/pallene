...
static lua_Number function_v_titan(
    ...
    (void)x4;
    {
        ...
        if (TITAN_LIKELY(
                x7 < sizenode(x1) &&
                (x8 = gnode(x1, x7),
                    (keyisshrstr(x8) && eqshrstr(keystrval(x8), x6))))) {
            x9 = gval(x8);
        } else {
            x9 = titan_lrecord_get_field(x1, x6, &x7);
        }
        if (TITAN_UNLIKELY(!ttisfloat(x9))) {
            titan_runtime_lrecord_type_error(
                L, "v", 6, LUA_TNUMFLT, rawtt(x9)
            );
        }
...
int luaopen_benchmarks_getv_cache1(lua_State *L)
{
    lua_checkstack(L, 4);
